...
// Event listener to open individual chat window
userList.addEventListener("click", (event) => {
  if (event.target.tagName === "LI") {
    const username = event.target.innerText;
    console.log("Selected Username:", username)
    const individualChatWindow = window.open(
      `ind_chat.html?user=${username}`,
      "_blank"
    );
    if (individualChatWindow) {
      individualChatWindow.focus();
    } else {
      alert(
        "Please allow pop-ups for this site to open the individual chat window."
      );
    }
  }
});
